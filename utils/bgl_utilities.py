...
UP_VECTOR_X = Vector((1, 0, 0))
UP_VECTOR_Y = Vector((0, 1, 0))
UP_VECTOR_Z = Vector((0, 0, 1))
...
def getPointBatch(context, useshader, location,*, color= (1, 0, 0, 1), pointsize=5):
    batch_type = 'POINTS' 
    batch = batch_for_shader(useshader, batch_type, {'pos': [location]})
    
    return {'type': batch_type, 'batch': batch, 'color': color, 'pointsize': 5}

def getHollowCircleBatch(context, useshader, location, normal,*, radius=5, resolution=20, color= (1, 0, 0, 1), linewidth=1):
    fl_resolution = float(resolution);

    positions = []
    positions_along_tangents = []
    positions_along_normals = []
    dot_x = UP_VECTOR_X.dot(normal);
    dot_y = UP_VECTOR_Y.dot(normal);
    dot_z = UP_VECTOR_Z.dot(normal);
    
    if(dot_x < 0.5):
        ortho_vector = normal.cross(UP_VECTOR_X).normalized() * radius
    elif(dot_y < 0.5):
        ortho_vector = normal.cross(UP_VECTOR_Y).normalized() * radius
    else:
        ortho_vector = normal.cross(UP_VECTOR_Z).normalized() * radius

    normal_v = normal * radius

    for i in range(resolution):        
        angle_radians = (float(i) / fl_resolution) * TWOPI;

        rot_m_tangent = Matrix.Rotation(angle_radians, 4, normal)
        rot_m_normal = Matrix.Rotation(angle_radians, 4, ortho_vector)
        
        position_tangent = location + (rot_m_tangent @ ortho_vector)
        position_normal = location + (rot_m_normal @ normal_v)

        positions_along_tangents.append(position_tangent)
        positions_along_normals.append(position_normal)
    
    positions.extend(positions_along_tangents)
    positions.extend(positions_along_normals)

    batch_type = 'LINE_LOOP'
    batch = batch_for_shader(useshader, 'LINE_LOOP', {'pos': positions})
    return {'type': batch_type, 'batch': batch, 'color': color, 'linewidth': linewidth }


def getPathBatch(context, useshader, path, *, color= (1, 0, 0, 1), pointsize=5):
    batch_type = 'LINE_STRIP'
    batch = batch_for_shader(useshader, batch_type, {'pos': path})
    return {'type': batch_type, 'batch': batch, 'color': color, 'pointsize': 5}

def getPathAsBatchesWithPoints(context, useshader, path, *, color= (1, 0, 0, 1), pointsize=5):
    batch_type = 'POINTS'
    batch = batch_for_shader(useshader, batch_type, {'pos': path})
    points_batch = {'type': batch_type, 'batch': batch, 'color': color, 'pointsize': pointsize}
    return points_batch

def getPathAsBatchesWithLines(context, useshader, path, *, color= (1, 0, 0, 1), linewidth=1):
    batch_type = 'LINE_STRIP'
    batch = batch_for_shader(useshader, batch_type, {'pos': path})
    line_batch = {'type': batch_type, 'batch': batch, 'color': color, 'linewidth': linewidth}
    return line_batch



def drawPoint2(location, color, size=15.0, enable_depth=False):
    if(enable_depth):
